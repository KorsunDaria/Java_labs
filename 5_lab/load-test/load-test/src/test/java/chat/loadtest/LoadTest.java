...
/**
 * Нагрузочные и стресс-тесты сервера.
 *
 * Нагрузочный тест  — проверяем пропускную способность при нормальной нагрузке.
 * Стресс-тест       — давим максимальным числом клиентов и сообщений,
 *                     проверяем что сервер не падает и не теряет сообщения.
 */
class LoadTest {
    ...
    void startServer() throws IOException {
        ...
        server.start();
        // небольшая пауза чтобы сервер успел встать
        sleep(100);
    }
    ...
    // -------------------------------------------------------------------------
    // Нагрузочный тест: один клиент, много сообщений — измеряем throughput
    // -------------------------------------------------------------------------
    ...
    void throughput_singleClient() throws Exception {
        ...
        try (TestClient client = server.connectClient("user_load")) {
            ...
            // минимальная планка: хотя бы 100 сообщений в секунду
    ...
    // -------------------------------------------------------------------------
    // Нагрузочный тест: много клиентов подключаются одновременно
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // Нагрузочный тест: N клиентов одновременно шлют сообщения
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // Стресс-тест: максимальное число клиентов
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // Стресс-тест: сервер не падает при быстрых подключениях и отключениях
    // -------------------------------------------------------------------------

    @Test
    @DisplayName("Стресс: быстрые подключения и отключения (flood)")
    void stress_connectDisconnectFlood() throws Exception {
        int rounds      = 50; // раундов
        int perRound    = 5;  // клиентов за раунд
        AtomicInteger errors = new AtomicInteger(0);
        ...
        for (int r = 0; r < rounds; r++) {
            ...
            sleep(20); // небольшой интервал между раундами
        }
    ...
    // -------------------------------------------------------------------------
    // Стресс-тест: сервер не теряет сообщения под нагрузкой
    // -------------------------------------------------------------------------
    ...
    void stress_noMessageLoss() throws Exception {
        ...
        // один получатель считает входящие
        TestClient receiver = server.connectClient("receiver");
        AtomicInteger received = new AtomicInteger(0);

        // setSoTimeout чтобы readMessage() не блокировал вечно в конце теста
        receiver.getSocket().setSoTimeout(2000);

        // читаем строго в одном потоке — иначе два потока на одном сокете перемешают XML
        Thread readerThread = Thread.ofVirtual().start(() -> {
            try {
                while (true) {
                    chat.protocol.Message msg = receiver.readMessage();
                    if (msg instanceof chat.protocol.EventMessageMsg) {
                        received.incrementAndGet();
                    }
                }
            } catch (IOException ignored) {} // таймаут или закрытие = конец чтения
        });
        ...
        // ждём пока все сообщения долетят до получателя:
        // опрашиваем счётчик каждые 200мс, выходим когда перестаёт расти
        int prevCount = -1;
        for (int i = 0; i < 20; i++) {   // максимум 4 секунды
            sleep(200);
            int cur = received.get();
            if (cur == prevCount) break;  // ничего нового не пришло — всё дошло
            prevCount = cur;
        }
